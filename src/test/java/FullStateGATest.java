...
public class FullStateGATest {
	private static final int[] INI_CHROM_0 = new int[256 / 16];
	private int[] field;
	private int[] chromo;
	...
	private void testCorrectIndexWithValues4(int fieldValue, int chromoIndex) {
		...
		chromo[chromoIndex / 16] |= 1 << (chromoIndex & 0B110);
		assertEquals(1, ga.getMoveDirection(field));
		chromo[chromoIndex / 16] = 0;
	}
	...
	public void testCrossover() {
		...
		FullStateGA child = new FullStateGA(4, states);
		ga.crossover(other, child);
	...
	public void testIllegalCrossover() {
		...
		ga.crossover(new FullStateGA(8, 8), new FullStateGA(4,4));
	}
}
